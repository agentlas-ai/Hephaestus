...
- **Stormbreaker auto-runner.** `hephaestus-network`가 runnable pipeline
  `execution_fabric`을 받으면 Stormbreaker를 자동으로 background 실행합니다.
  계획만 보고 싶으면 `--plan-only`를 붙입니다.
- **새 터미널 명령 `hephaestus-storm`.** `hephaestus-storm "<request>"
  --background`로 Stormbreaker를 직접 실행할 수 있고, 결과와 로그는
  `.agentlas/stormbreaker/background/<run_id>/` 아래에 남습니다.
- **Packet 실행 receipt.** 각 packet은 contract, stdout, stderr, result JSON,
  execution receipt, final-gate status를 남깁니다.
- **Executor adapter.** `--executor-command`, `--session-inventory`,
  `--max-workers`, packet별 `--timeout`으로 Codex, Claude, GLM, DeepSeek,
  Gemini, local model lane을 연결할 수 있습니다.
- **Elastic but bounded.** 어려운 작업은 가능한 session lane으로 분산하지만,
  무제한 swarm을 만들거나 dependency join/final gate를 우회하지 않습니다.
